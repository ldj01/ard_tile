...
def set_scene_to_inqueue(connection, scene_id):
    ...
    updatesql = ("update ARD_PROCESSED_SCENES "
                 "set PROCESSING_STATE = 'INQUEUE' "
                 "where scene_id = :scene_id")
    update(connection, updatesql, scene_id=scene_id)


def reset_records(connection):
    """Set all in-progress scenes to 'BLANK' processing status."""
    updatesql = ("update ARD_PROCESSED_SCENES "
                 "set PROCESSING_STATE = 'BLANK', DATE_PROCESSED = sysdate "
...
def duplicate_scenes_insert(connection, dup_scene_list):
    """Insert duplicate scenes into ARD_PROCESSED_SCENES as 'DUPLICATE'."""
    insert_sql = ("insert /*+ ignore_row_on_dupkey_index"
                  "(ARD_PROCESSED_SCENES, SCENE_ID_PK) */ "
                  "into ARD_PROCESSED_SCENES (scene_id,file_location,"
                  "PROCESSING_STATE) values (:1,:2,:3)")
    if dup_scene_list:
        logger.info("Duplicate scenes inserted into ARD_PROCESSED_SCENES "
                    "table: %s", dup_scene_list)
        update_many(connection, insert_sql, dup_scene_list)
...
def processed_scenes(connection, completed_scene_list):
    ...
    insert_sql = ("insert /*+ ignore_row_on_dupkey_index"
                  ...
                  "into ARD_PROCESSED_SCENES (scene_id,file_location) "
                  "values (:1,:2)")
    update_many(connection, insert_sql, completed_scene_list)
